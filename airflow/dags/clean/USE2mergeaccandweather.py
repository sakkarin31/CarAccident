...
def process_year(year: int):
    ...
    # รองรับหลายชื่อคอลัมน์เวลา
    time_col = None
    for cand in ["เวลา", "เวลาเกิดเหตุ", "time", "Time", "เวลา_เกิดเหตุ"]:
        if cand in dfa.columns:
            time_col = cand
            break
    if time_col is None:
        raise KeyError("ไม่พบคอลัมน์เวลาในไฟล์อุบัติเหตุ")

    dta_time = parse_time_flexible(dfa[time_col])
